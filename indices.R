...
## CDD. Maximum number of consecutive days with TMAX above t_thresh
calc_cdd_temp_m <- function(TMEAN, t_thresh=17.8){
  runs <- rle(TMEAN > t_thresh)
  cons_days <- max(runs$lengths[runs$values==1], na.rm=TRUE)
  return(cons_days)
}
calc_cdd_tempCMean <- compiler::cmpfun(calc_cdd_temp_m)
...
## NT35. Number of days with max. temperature above 35?C
calc_hts_mean <- function(tmean, t_thresh=35) {
  hts <- length(which(tmean >= t_thresh))
  return(hts)
}
calc_htsCMP_mean <- compiler::cmpfun(calc_hts_mean)
