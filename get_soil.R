...
# shp_fl <- paste0('//catalogue/Workspace14/WFP_ClimateRiskPr/1.Data/shps','/',tolower(country),'/',tolower(iso),'_regions','/',tolower(iso),'_regions.shp') 
# shp_fl <- "//CATALOGUE/Workspace14/WFP_ClimateRiskPr/1.Data/shps/world_shapefile/North_America/north_america.shp"
root_depth <- 60 
...
get_soil <- function(shp_fl, root_depth, outfiles){
  ...
  if(sum(!file.exists(outfiles)) != 0){
    ...
    raster::writeRaster(x = sscp, filename = paste0(outfiles,'/',iso,'_','scp','.tif'), overwrite = T)
    raster::writeRaster(x = ssat, filename = paste0(outfiles,'/',iso,'_','ssat','.tif'), overwrite = T)
